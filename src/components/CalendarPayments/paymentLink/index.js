...
const PaymentLink = ({ date, amount, loan, type = null, index }) => {
  ...
  const PaymentLoan = () => {
    return loan ? (
      <div className={paymentLoan}>
        <p
          className={paymentParraf}
          style={{ color: textColorType(), whiteSpace: "nowrap" }}
        >
          {`$ ${formatNumber(loan)}`}
        </p>
      </div>
  ...
  return (
    <div
      className={`${paymentLink} ${index === 0 ? firstPayment : null}`}
      style={{
        background:
          loan === undefined
            ? backgroundColorType()
